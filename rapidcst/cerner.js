...
function ShowOrders(tjson) {
	...
	for (var x=0; x<tjson.length; x++) {
		...
		if (obj.type == "order") {
			tvalue = obj.value;
			tHTML += MakeOrder(obj.data, "", currCat, tvalue);
		}
...
function MakeOrder(torder, tsentence, ttype, tvalue) {
	...
	if (tsentence == "") {
		tsentence = "Select an order sentence";
	}
	...
	tHTML += '			</div>' + "\n";
	tHTML += '			<div class="order_text">' + torder + '</div>' + "\n";
	tHTML += '			<div class="sentence_text">' + tsentence + '</div>' + "\n";
	tHTML += '		</div>' + "\n";
